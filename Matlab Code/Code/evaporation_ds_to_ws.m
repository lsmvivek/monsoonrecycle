    ...
    for i = 1:17415
        if (dry_data(1,4,k) == evapo(1,2,i) && dry_data(1,5,k) == evapo(1,3,i))
            ...
            for j = 1:length(dry_data(:,1,k))    
                if (dry_data(j,1,k) ~= 0 && dry_data(j,3,k) >start_date && dry_data(j,3,k)< end_date )     % May 31 till October 15
...
for i = 1:length(evp(1,1,:))
    alpha(:,1,i) = evp(:,1,i);                                              % Y cumulative evp in DS 
    alpha(:,2,i) = evp(:,2,i);                                              % X1 cumulative rain is WS
    alpha(:,3,i) = evp(:,3,i);                                              % X2 WS spell length
end
            ...
            if evp(1,10,i)== start_date + 1
               ...
               [corrmat(i,2) pvalmat(i,2)] = corr(y1,x2,"tail","both","type","Kendall");
                ...
                if length(y1) == 1
                    ...
                    [corrmat(i,1) pvalmat(i,1)] = corr(y1(1:end-1),x1(2:end),"tail","both","type","Kendall");
                    [corrmat(i,2) pvalmat(i,2)] = corr(y1(1:end-1),x2(2:end),"tail","both","type","Kendall");
                end
            end
        elseif length(y1) == length(x1) -1   
               [corrmat(i,1) pvalmat(i,1)] = corr(y1,x1(2:end),"tail","both","type","Kendall");
               [corrmat(i,2) pvalmat(i,2)] = corr(y1,x2(2:end),"tail","both","type","Kendall");
        elseif length(y1) -1 == length(x1)   
               [corrmat(i,1) pvalmat(i,1)] = corr(y1(1:end-1),x1,"tail","both","type","Kendall");
               [corrmat(i,2) pvalmat(i,2)] = corr(y1(1:end-1),x2,"tail","both","type","Kendall");
        end
    else
         corrmat(i,1) = -999;
         pvalmat(i,1) = -999;
